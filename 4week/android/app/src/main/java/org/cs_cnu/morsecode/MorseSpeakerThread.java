...
public class MorseSpeakerThread extends Thread {
    ...
    public void run() {
        ...
        int ptr = 0;
// Need to edit below
        Log.i("arrsize", String.valueOf(array_size));
        for (String str : iterator) {
            switch (str) {
                ...
                case "-": // 긴 음, 3unit
                    ...
                    break;
                case " ": // 문자 사이, 3unit, 3 -  1 - 1
                    for (int i = 0; i < unit_size; i++) {
                        samples[ptr] = (short) 0;
                        ptr = ptr + 1;
                    }
                    break;
                case "/": // 단어 사이 구분, 7unit, 7 - 1
                    for (int i = 0; i < unit_size*6; i++) {
                        samples[ptr] = (short) 0;
                        ptr = ptr + 1;
                    }
            }
            for (int i = 0; i < unit_size; i++) { // 모스부호 사이 구분, 1unit
                samples[ptr] = (short) 0;
                ptr = ptr + 1;
            }
        }
